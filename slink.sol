...
/**
 *    A Chainlink pegged base-money that is
...
contract PriceConsumerV3 {
     ...
     */
    constructor() public {
        priceFeed = AggregatorV3Interface(0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE); //eth main net : 0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419
    }
...
contract Slink is Context, IERC20, Ownable {
    ...
    IPancakeswapV2Router02 public immutable pancakeV2Router;
    address public pancakeBnbPool;
    address[] public pancakePairPath;
    ...
    uint256 private constant PEGGED_PRICE = 300e6;                                                // $3.00
    ...
    uint256 private _rebase_caller_fee = 25e18;
    ...
    constructor (IPancakeswapV2Router02 _pancakeV2Router) public {
        ...
        pancakeBnbPool = IPancakeswapV2Factory(_pancakeV2Router.factory())
            .createPair(address(this), _pancakeV2Router.WETH());
            
        pancakePairPath = new address[](2);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(block.number == _lastRebaseBlock)
            require(sender == address(this), 'SLINK: the transaction is not allowed at epoch block.');
            
        if(sender != pancakeBnbPool)
            require(!blacklist[recipient] && !blacklist[sender], 'SLINK: the transaction was blocked.');
        
        if(sender == pancakeBnbPool && !_live)
    ...
    function Supersize() public returns(uint256) {
        require(_live && block.timestamp >= _lastEpochTime + _epochCycle, 'SLINK: early epoch.');
        ...
        IPancakeswapV2Pair(pancakeBnbPool).sync();
    ...
    function updateRebaseLag(uint256 rebaseLag) public onlyOwner {
        require(rebaseLag > 0, 'SLINK: rebase lag is 0');
    ...
    function getTokenPriceInBNB() public view returns(uint256) {
        return pancakeV2Router.getAmountsIn(1e18, pancakePairPath)[0];
    }
    ...
    function getTokenPriceInUSD() public view returns(uint256) {
        ...
        uint256 BNBPriceInUSD = uint256(_priceContract.getLatestPrice());
        return tokenPriceInBNB.mul(BNBPriceInUSD);
    }
